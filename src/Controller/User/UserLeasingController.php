...
class UserLeasingController extends AbstractController
{
     ...
     */
    public function userVehicleList(VehicleRepository $vehicleRepository)
    {
        $user = $this->getUser();
     ...
     */
    public function userSingleAnnonce(Request $request, VehicleRepository $vehicleRepository)
    {
     ...
     */
    public function acceptBooking(Request $request, ObjectManager $em, BookingRepository $bookingRepository, DateRepository $dateRepository, \Swift_Mailer $mailer)
    {
        $token = $request->request->get('token');
        $bookingId = $request->request->get('bookingId');
        $bookingRenter = $bookingRepository->find($bookingId);
        ...
        if ($this->isCsrfTokenValid('accept', $token) && $bookingRenter->getOwner() == $this->getUser()){
            ...
            if($datesId === $bookingRenter->getDateRenter()){
                ...
                foreach($datesId as $date){
                    $em->remove($dateRepository->find($date));
                }
                $em->flush();
     ...
     */
    public function denyBooking(Request $request, BookingRepository $bookingRepository, ObjectManager $em,  \Swift_Mailer $mailer)
    {
        $token = $request->request->get('token');
        $bookingId = $request->request->get('bookingId');
        $bookingRenter =  $bookingRepository->find($bookingId);
    ...
    public function vehicleReception(Request $request, BookingRepository $bookingRepository, ObjectManager $em, \Swift_Mailer $mailer)
    {
        $token = $request->request->get('token');
        $bookingId = $request->request->get('bookingId');
     ...
     */
    public function vehicleLeasingForm(Request $request, VehicleRepository $vehicleRepository, ObjectManager $em)
    {
        $user = $this->getUser();
        $vehicles = $vehicleRepository->findBy([
            'user' => $user,
        ...
        if ($this->isCsrfTokenValid('date-form', $token)){
            ...
            $datVehicle = $vehicleRepository->find($datVehicleId);
    ...
    */
    public function deletePost(Request $request, VehicleRepository $vehicleRepository, BookingRepository $bookingRepository, ObjectManager $em)
    {
        $token = $request->request->get('token');
     ...
     */
    public function showRental(Request $request, BookingRepository $bookingRepository, PaginatorInterface $paginator)
    {
        $bookings = $bookingRepository->findByRenter($this->getUser());
     ...
     */
    public function showOwnerHistoric(Request $request, BookingRepository $bookingRepository, PaginatorInterface $paginator)
    {

        $bookings = $bookingRepository->findByOwner($this->getUser());
     ...
     */
    public function addReview(Request $request, ObjectManager $em, Booking $booking, CommentRepository $commentRepository, \Swift_Mailer $mailer)
    {
        $comment = new Comment;
    ...
    if ($booking->getOwner() == $this->getUser() || $booking->getRenter() == $this->getUser()) {
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $allRate = $commentRepository->findBy(['customer' => $comment->getCustomer()]);
    ...
    */
    public function deleteVehicle(Request $request, BookingRepository $bookingRepository, VehicleRepository $vehicleRepository, DateRepository $dateRepository, ObjectManager $em)
    {
        $user = $this->getUser(); 
        $token = $request->request->get('token');
        ...
        if ($this->isCsrfTokenValid('delete-vehicle', $token) && $user == $vehicle->getUser()){
            $bookings = $bookingRepository->findBy(['vehicle' => $vehicle]);

            if ($bookings) {
                foreach($bookings as $booking) {
                    if($booking->getStatus() != 2){
                        $vehicle->setStatus(0);
                        $dates = $dateRepository->findBy(['vehicle' => $vehicle]);
